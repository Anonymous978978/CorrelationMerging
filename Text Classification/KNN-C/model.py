...
class SCLModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        loss_con = (pos_scores.masked_fill(no_neg_mask, 0) / (neg_scores + 1e-12)).masked_fill(N_mask,
                                                                                               0).sum() / P_mask.sum()
    ...
    def forward(self,
        ...
        if mode == 'train':
            if self.train_multi_head:  # False
                return self.forward_by_multi_head(query=query, positive_sample=positive_sample,
            ...
            return torch.argmax(probs, dim=1).tolist(), labels.cpu().numpy().tolist(), logits_cls
            ...
            probs = torch.softmax(logits_cls, dim=1)
            return probs, seq_embed, logits_cls
        else:
...
class ContrastiveMoCoKnnInitByBert(nn.Module):
    ...
    def forward_by_multi_head(self,
        ...
        if loss_cls is None:
            return loss_con
        return loss_con * self.config.contrastive_rate_in_training + loss_cls * (
                1 - self.config.contrastive_rate_in_training)
