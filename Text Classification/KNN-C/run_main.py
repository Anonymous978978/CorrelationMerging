...
label_map_to_domain = {}
for k, v in label_domain.items():
    for each in v:
        label_map_to_domain[each] = k
...
@dataclass
class ArgArguments:
    use_roberta: bool = field(
        default=False,
        metadata={"help": "use roberta or not"})
    show_lof: bool = field(
        default=False,
        metadata={"help": "show performance of lof"})
    group_strategy: str = field(
        default="taxonomy",
        metadata={"help": "group strategies, it should in [correlation, confusion, taxonomy]"}
    )
    scoring_function: str = field(
        default="sl",
        metadata={"help": "scoring functions, it should be in [sl, sp, msp]"}
    )
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, TrainingArguments, FitLogArguments, ArgArguments))

    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        # If we pass only one argument to the script_v0 and it's the path to a json file,
        # let's parse it to get our arguments.
        model_args, data_args, training_args, fitlog_args, args = parser.parse_json_file(
            json_file=os.path.abspath(sys.argv[1]))
    else:
        model_args, data_args, training_args, fitlog_args, args = parser.parse_args_into_dataclasses()
    ...
    if "clinc" not in training_args.data:
        ...
        seen_labels = np.random.choice(unique_labels, int(len(unique_labels) * training_args.known_ratio),
                                       replace=False)
        # test another method
    ...
    data = dict()
    training_args.debug = not True
    if training_args.debug:
        df_train_seen, df_valid_seen, df_valid_oos, df_test = df_train_seen[::10], df_valid_seen[::10], df_valid_oos[
                                                                                                        ::10], df_test[
                                                                                                               ::10]
    ...
    # Training
    if False and training_args.do_train:
        ...
        pass
        # if args.use_roberta:
        #     model = torch.load("clinc_full_knn_model_roberta.pth")
        # else:
        #     model = torch.load("clinc_full_knn_model_bert.pth")
    ...
    if training_args.do_predict:
        ...
        evaler.myevaluation(args, model_path=model_args.model_name_or_path)
