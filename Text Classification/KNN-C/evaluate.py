...
class Evaluation:
    ...
    def __init__(self,
        ...
        self.new_label_list = new_label_list
        if args is None:
    ...
    def myevaluation(self, args, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        predict = []
        target = []
        ...
        """with torch.no_grad():
            ...
            if args.group_strategy == "correlation":
                print("use correlation matrix")
                correlated_classes_among_class = [[] for i in range(class_num)]
                W = model.classifier_liner.out_proj.weight.data
        ...
        #               "prob_valid_ood": prob_valid_ood,
        #               "prob_test": prob_test,
        #               # "valid_all_feature": valid_all_feature,
        #               "valid_all_prob": valid_all_prob,
        #               "valid_all_labels": valid_all_labels,
        #               }
        # torch.save(saved_data, "{}-feature_prob_logit".format(model_path))
        # saved_data = {"correlated_classes_among_class": correlated_classes_among_class}
        # torch.save(saved_data, "{}-{}-{}".format(model_path, args.scoring_function, args.group_strategy))

        saved_data = torch.load("{}-feature_prob_logit".format(model_path))
        logit_test = saved_data["logit_test"]
        labels_test = saved_data["labels_test"]
        prob_train = saved_data["prob_train"]
        prob_valid = saved_data["prob_valid"]
        prob_valid_ood = saved_data["prob_valid_ood"]
        prob_test = saved_data["prob_test"]
        valid_all_prob = saved_data["valid_all_prob"]
        valid_all_labels = saved_data["valid_all_labels"]
        if args.scoring_function not in ["sl", "sp"]:
            pass
        else:
            saved_data = torch.load("{}-{}-{}".format(model_path, args.scoring_function, args.group_strategy))
        ...
        if args.scoring_function == "sl":
            ...
            if args.group_strategy == "taxonomy":
                oos_logit = logit_test[labels_test == 150].detach().cpu()[:, self.new_label_list]
                id_logit = logit_test[labels_test != 150].detach().cpu()[:, self.new_label_list]
            else:
                oos_logit = logit_test[labels_test == 150].detach().cpu()
                id_logit = logit_test[labels_test != 150].detach().cpu()
            ...
            if args.group_strategy == "taxonomy":
                oos_probs = prob_test[labels_test == 150].detach().cpu()[:, self.new_label_list]
                id_probs = prob_test[labels_test != 150].detach().cpu()[:, self.new_label_list]
            else:
                oos_probs = prob_test[labels_test == 150].detach().cpu()
                id_probs = prob_test[labels_test != 150].detach().cpu()
            oos_msp_ids = oos_probs.max(dim=-1)[1].detach().cpu().numpy()
            id_msp_ids = id_probs.max(dim=-1)[1].detach().cpu().numpy()
            if args.group_strategy == "taxonomy":
                oos_msp = np.array(
                    [sum(op[(oid // 15) * 15:(oid // 15 + 1) * 15]).item() for op, oid in
                     zip(oos_probs, oos_msp_ids)])
                id_msp = np.array(
                    [sum(op[(oid // 15) * 15:(oid // 15 + 1) * 15]).item() for op, oid in
                     zip(id_probs, id_msp_ids)])
            else:
                oos_msp = np.array([sum(op[correlated_classes_among_class[oid]]).item() for op, oid in
                                    zip(oos_probs, oos_msp_ids)])
                id_msp = np.array(
        ...
        oos_score = norm_lof_score[labels_test == 150]
        id_score = norm_lof_score[labels_test != 150]
