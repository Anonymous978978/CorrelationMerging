...
class BiLSTM(nn.Module):
    def __init__(self, embedding_matrix, BATCH_SIZE, HIDDEN_DIM, CON_DIM, NUM_LAYERS, n_class_seen, DO_NORM, ALPHA,
                 BETA, OOD_LOSS, ADV, CONT_LOSS, norm_coef, cl_mode=1, lmcl=True, use_cuda=True, use_bert=False,
                 sup_cont=False):
        ...
        if self.use_bert:
            ...
            self.rnn = nn.GRU(input_size=embedding_matrix.shape[1], hidden_size=self.hidden_dim,
                              num_layers=self.num_layers,
                              batch_first=True, bidirectional=True).to(self.use_cuda)
    ...
    def forward(self, seq, adv_features=None, label=None, sim=None, mode='ind_pre'):
        if mode == 'ind_pre' or mode == 'finetune':
            if self.use_bert:
                seq_embed = self.bert_model(
                    **self.bert_tokenizer(seq, return_tensors='pt', padding=True, truncation=True).to(self.use_cuda))[0]
                seq_embed = self.dropout(seq_embed)
...
class BiLSTM_roberta(nn.Module):
    def __init__(self, embedding_matrix, BATCH_SIZE, HIDDEN_DIM, CON_DIM, NUM_LAYERS, n_class_seen, DO_NORM, ALPHA,
        ...
        if self.use_bert:
            ...
            self.bert_model = RobertaModel.from_pretrained('roberta-base').to(self.use_cuda)
