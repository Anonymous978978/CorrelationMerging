...
# Modeling
import torch
from model import BiLSTM, BiLSTM_roberta
...
def parse_args():
    ...
    parser.add_argument("--dataset", type=str, default="CLINC_OOD", choices=["CLINC", "CLINC_OOD"],
    ...
    parser.add_argument("--model_dir", type=str, default="CLINC_OOD-100-vallian-lmlc",
    ...
    parser.add_argument("--use_bert", action="store_false",
    ...
    parser.add_argument('--use_roberta', action="store_true", help='roberta or bert')
    parser.add_argument('--group_strategy', type=str, default="taxonomy",
                        help='group strategies, it should in [correlation, confusion, taxonomy]')
    parser.add_argument('--scoring_function', type=str, default="sl",
                        help='scoring functions, it should be in [sl, sp, msp]')
    parser.add_argument('--show_lof', action="store_true", help="whether to show lof performance")

    args = parser.parse_args()
    return args
args = parse_args()
assert args.group_strategy
...
label_sort_by_domain_list = [each for k, v in label_domain.items() for each in v]
for l in label_sort_by_domain_list:
    new_label_list.append(label_list.index(l))
...
if not args.unseen_proportion:
    ...
    train_data = (
        np.concatenate((X_train_seen, X_train_ood), axis=0), np.concatenate((y_train_onehot, y_train_ood), axis=0))
    valid_data = (
        np.concatenate((X_valid_seen, X_valid_ood), axis=0), np.concatenate((y_valid_onehot, y_valid_ood), axis=0))
...
if args.mode in ["train", "both"]:
    ...
    if not use_roberta:
        ...
        model = BiLSTM(embedding_matrix, BATCH_SIZE, HIDDEN_DIM, CON_DIM, NUM_LAYERS, n_class_seen, DO_NORM, ALPHA,
                       BETA, OOD_LOSS, ADV, CONT_LOSS, NORM_COEF, CL_MODE, LMCL, use_bert=USE_BERT, sup_cont=SUP_CONT,
                       use_cuda=CUDA)
    else:
        print("use roberta-base model")
        filepath = os.path.join(output_dir, 'roberta_model_best.pkl')
        model = BiLSTM_roberta(embedding_matrix, BATCH_SIZE, HIDDEN_DIM, CON_DIM, NUM_LAYERS, n_class_seen, DO_NORM,
                               ALPHA, BETA, OOD_LOSS, ADV, CONT_LOSS, NORM_COEF, CL_MODE, LMCL, use_bert=USE_BERT,
                               sup_cont=SUP_CONT, use_cuda=CUDA)
...
if args.mode in ["test", "both", "find_threshold"]:
    ...
    if args.n_plus_1:
        ...
        model_name = "model_best.pkl" if not use_roberta else "roberta_model_best.pkl"
        ...
        if args.group_strategy == "correlation":
            ################################
            # Based on correlation intensity
            correlated_classes_among_class = [[] for i in range(class_num)]
            W = model.fc.weight.data
            correlation_matrix = torch.matmul(W, W.T).cpu()
            for i in range(class_num):
                correlated_classes_among_class[i] = correlation_matrix[i].topk(15)[1].tolist()
            correlated_classes_among_class = torch.tensor(correlated_classes_among_class)
            ################################
        elif args.group_strategy == "confusion":
            prob_list_among_class = [[] for i in range(150)]
            correlated_classes_among_class = [[] for i in range(150)]
            prob_train_predictions = torch.argmax(prob_train, dim=-1)
            for i in range(150):
                prob_list_among_class[i] = prob_train[prob_train_predictions==i].sum(dim=0).tolist()
                # prob_list_among_class[i][i] = 0
            prob_list_among_class = torch.tensor(prob_list_among_class)
            for i in range(150):
                correlated_classes_among_class[i] = prob_list_among_class[i].topk(15)[1].tolist()
            correlated_classes_among_class = torch.tensor(correlated_classes_among_class)
        else:
            correlated_classes_among_class = None
            pass
        ...
        for j, (seq, label) in enumerate(test_loader):
            if labels_test is None:
                labels_test = list(label)
            else:
                labels_test = labels_test + list(label)
            ...
            if feature_test != None:
                ...
                logit_test = torch.cat((logit_test, output[-1]), dim=0)
            else:
        ...
        saved_data = {"logit_test": logit_test,
                      "labels_test": labels_test,
                      "prob_test": prob_test,
                      # "feature_train": feature_train,
                      # "feature_valid": feature_valid,
                      # "feature_valid_ood": feature_valid_ood,
                      # "feature_test": feature_test,
                      "prob_test": prob_test,
                      # "valid_all_feature": valid_all_feature,
                      }
        torch.save(saved_data, "{}-feature_prob_logit".format(model_name))
        saved_data = {"correlated_classes_among_class": correlated_classes_among_class}
        torch.save(saved_data, "{}-{}-{}-{}".format(model_name, model_dir, args.scoring_function, args.group_strategy))"""

        if args.scoring_function not in ["sl", "sp"]:
            saved_data = torch.load("{}-{}-feature_prob_logit".format(model_name, args.model_dir))
            logit_test = saved_data["logit_test"]
            labels_test = saved_data["labels_test"]
            ...
            labels_test = saved_data["labels_test"]
            prob_test = saved_data["prob_test"]
            saved_data = torch.load("{}-{}-{}-{}".format(model_name, model_dir, args.scoring_function, args.group_strategy))
            correlated_classes_among_class = saved_data["correlated_classes_among_class"]

        if args.scoring_function == "sl":
            # Superlogit
            alpha = 14 / 15
            if args.group_strategy == "taxonomy":
                oos_logit = logit_test[labels_test == "unseen"].detach().cpu()[:, new_label_list]
                id_logit = logit_test[labels_test != "unseen"].detach().cpu()[:, new_label_list]
            else:
                oos_logit = logit_test[labels_test == "unseen"].detach().cpu()
                id_logit = logit_test[labels_test != "unseen"].detach().cpu()
            oos_logit_ids = oos_logit.max(dim=-1)[1].detach().cpu().numpy()
            id_logit_ids = id_logit.max(dim=-1)[1].detach().cpu().numpy()

            for op, oid in zip(oos_logit, oos_logit_ids):
                if args.group_strategy == "taxonomy":
                    bros = op[(oid // 15) * 15:(oid // 15 + 1) * 15]
                else:
                    bros = op[correlated_classes_among_class[oid]]
                op[oid] = sum(bros) * (1 - alpha) + op[oid]
            for op, oid in zip(id_logit, id_logit_ids):
                if args.group_strategy == "taxonomy":
                    bros = op[(oid // 15) * 15:(oid // 15 + 1) * 15]
                else:
                    bros = op[correlated_classes_among_class[oid]]
            ...
            oos_logit, id_logit = oos_logit.numpy(), id_logit.numpy()
            maxest = max(np.max(oos_logit).item(), np.max(id_logit).item())
            minest = min(np.min(oos_logit).item(), np.min(id_logit).item())
            oos_logit = torch.from_numpy((oos_logit - minest) / (maxest - minest))
            id_logit = torch.from_numpy((id_logit - minest) / (maxest - minest))
            oos_energy = oos_logit.max(dim=-1)[0].detach().cpu().numpy()
            id_energy = id_logit.max(dim=-1)[0].detach().cpu().numpy()

            y = np.concatenate((np.ones_like(oos_energy), np.zeros_like(id_energy)))
            scores = np.concatenate((oos_energy, id_energy))
            fpr_tpr95 = ErrorRateAt95Recall1(y, 1 - scores) * 100
            auroc = 100 * roc_auc_score(y, 1 - scores)
            print("{}-{}: AUROC {}, FPR95:{}".format(args.scoring_function, args.group_strategy, auroc, fpr_tpr95))
        elif args.scoring_function == "sp":
            if args.group_strategy == "taxonomy":
                oos_probs = prob_test[labels_test == "unseen"].detach().cpu()[:, new_label_list]
                id_probs = prob_test[labels_test != "unseen"].detach().cpu()[:, new_label_list]
            else:
                oos_probs = prob_test[labels_test == "unseen"].detach().cpu()
                id_probs = prob_test[labels_test != "unseen"].detach().cpu()
            oos_msp_ids = oos_probs.max(dim=-1)[1].detach().cpu().numpy()
            id_msp_ids = id_probs.max(dim=-1)[1].detach().cpu().numpy()
            if args.group_strategy == "taxonomy":
                oos_msp = np.array(
                    [sum(op[(oid // 15) * 15:(oid // 15 + 1) * 15]).item() for op, oid in zip(oos_probs, oos_msp_ids)])
                id_msp = np.array(
                    [sum(op[(oid // 15) * 15:(oid // 15 + 1) * 15]).item() for op, oid in zip(id_probs, id_msp_ids)])
            else:
                oos_msp = np.array(
                    [sum(op[correlated_classes_among_class[oid]]).item() for op, oid in zip(oos_probs, oos_msp_ids)])
                id_msp = np.array(
                    [sum(op[correlated_classes_among_class[oid]]).item() for op, oid in zip(id_probs, id_msp_ids)])
            ...
            oos_probs = prob_test[labels_test == "unseen"].detach().cpu()
            id_probs = prob_test[labels_test != "unseen"].detach().cpu()
            oos_msp = oos_probs.max(dim=-1)[0].detach().cpu().numpy()
            id_msp = id_probs.max(dim=-1)[0].detach().cpu().numpy()
            y = np.concatenate((np.ones_like(oos_msp), np.zeros_like(id_msp)))
            scores = np.concatenate((oos_msp, id_msp))
            fpr_tpr95 = ErrorRateAt95Recall1(y, 1 - scores) * 100
            auroc = 100 * roc_auc_score(y, 1 - scores)
        ...
        for setting in settings:
            ...
            if ood_method == "lof":
                ...
                if args.show_lof:
                    ...
                    norm_lof_score = (lof_score - min(lof_score)) / (max(lof_score) - min(lof_score))

                    oos_score = norm_lof_score[labels_test == "unseen"]
                    id_score = norm_lof_score[labels_test != "unseen"]

                    y = np.concatenate((np.ones_like(oos_score), np.zeros_like(id_score)))
                    scores = np.concatenate((oos_score, id_score))
                    fpr_tpr95 = ErrorRateAt95Recall1(y, 1 - scores) * 100
                    auroc = 100 * roc_auc_score(y, 1 - scores)
                    print("LOF: AUROC {}, FPR95:{}".format(auroc, fpr_tpr95))
                ...
                f, acc, f_seen, acc_in, p_seen, r_seen, f_unseen, acc_ood, p_unseen, r_unseen = get_score(cm)
                plot_confusion_matrix(pred_dir, cm, classes, normalize=False, figsize=(9, 6),
                                      title=method + ' on ' + dataset + ', f1-macro=' + str(f))
                print(cm)
                log_pred_results(f, f_seen, f_unseen, p_unseen, r_unseen, classes, pred_dir, cm, OOD_LOSS, ADV,
                                 CONT_LOSS)
            elif ood_method == "gda":
